  ...
  <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-8">
      <div class="flex items-center">
        <NuxtLink :to="`/roles/${roleId}`" class="text-gray-400 hover:text-gray-600 mr-4">
    ...
    <div v-else-if="rolesStore.error" class="text-center py-8">
      <div class="text-red-600 mb-4">{{ rolesStore.error }}</div>
      ...
      <div class="card">
        <form @submit.prevent="updateRole" class="space-y-6">
          ...
          <div class="flex justify-end space-x-4 pt-6 border-t border-gray-200">
            <NuxtLink :to="`/roles/${roleId}`" class="btn-secondary">
              Cancel
            </NuxtLink>
            <button
              type="submit"
              ...
              class="btn-primary"
            >
              {{ rolesStore.loading ? 'Updating...' : 'Update Role' }}
            </button>
          </div>
        </form>
      </div>
...
const updateRole = async () => {
  console.log('Updating role:', roleId.value, form)
  ...
  const result = await rolesStore.updateRole(roleId.value, form)
  ...
  if (result.success) {
    ...
    if (result.error && typeof result.error === 'object') {
      ...
      errors.general = result.error || 'Failed to update role'
    }
...
// Lifecycle
onMounted(async () => {
  console.log('Role edit page mounted, roleId:', roleId.value)
  ...
  if (roleId.value && roleId.value > 0) {
    ...
    if (role.value) {
      ...
      form.display_name = role.value.display_name || ''
      form.description = role.value.description || ''
      form.permissions = role.value.permissions ? role.value.permissions.map(permission => permission.id) : []
      console.log('Form populated:', form)
    }
  } else {
    console.error('Invalid roleId:', roleId.value)
  }
})
...
watch(role, (newRole) => {
  if (newRole) {
    ...
    form.permissions = newRole.permissions ? newRole.permissions.map(permission => permission.id) : []
    console.log('Form updated from role change:', form)
  }
})
</script>
